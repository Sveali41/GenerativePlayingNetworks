...
class Trainer(object):
    ...
    def __init__(self, gen, agent, save, version=0, elite_mode='max', elite_persist=True):
        ...
        self.my_dir = path.LEVEL

        self.save_paths = {'dir': save, 'agent': os.path.join(save, 'agents'), 'models': os.path.join(save, 'models'),
                           'levels': os.path.join(save, 'levels.csv'), 'loss': os.path.join(save, 'losses.csv')}
    ...
    def train(self, updates, batch_size, gen_batches, div_batches, rl_steps, pretrain):
        ...
        for update in range(self.version + 1, self.version + int(updates) + 1):
            ...
            else:  # if there is no pretrained env
                self.new_elite_levels(z(batch_size))
                # self.agent.set_envs(self.temp_dir.name)
                self.agent.set_envs(self.my_dir)
                self.agent.train_agent(rl_steps)  # train the agent on the default env
            ...
            for i in range(gen_batches + div_batches):
                ...
                generated_levels = lvl_imgs  # sample a new level
                # visualize
                # generated_levels = ' '.join(map(str, generated_levels))
                # pic = self.level_visualizer.draw_level(generated_levels)
                # print(pic)
