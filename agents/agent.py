...
class Agent:
    ...
    def __init__(self, env_def, processes=1, dir='.', version=0, lr=2e-4, architecture='base', dropout=0,
                 reconstruct=None, r_weight=.05):
        ...
        path = Paths
        self.level_path = path.LEVEL
        self.envs = None
        self.num_envs = -1
        # self.set_envs(num_envs=1)
        self.set_envs(self.level_path, num_envs=1)
        ...
        if (self.reconstruct):
            ...
            self.r_loss = lambda pred, true: -r_weight * (true * torch.log(pred.clamp(min=1e-7, max=1 - 1e-7))).sum(
                dim=1).mean()
    ...
    def set_envs(self, level_path=None, num_envs=None):
        # the default num_envs=None -> num_envs = num_envs (when the input num_envs is not None)
        # /else num_envs is  self.num_processes = 1
        ...
        if (level_path != self.level_path or self.envs is None or num_envs != self.num_envs):
            # 1. input level_path != self.level_path
            # or 2. self.envs is None
            # or 3. num_envs != self.num_envs
            if (self.envs is not None):
                # if self.envs != None
                self.envs.close()
            self.level_path = level_path  # the level saved path
            self.envs = make_vec_envs(self.env_def, level_path, self.seed, num_envs, self.gamma, self.log_dir,
                                      self.device, True)
        # set the self.num_envs = num_envs, then don't need to make the vec env again
